    ...
    public class Agent : MonoBehaviour
    {
        ...
        private IEnumerator BoidsBehavior()
        {
            ...
            while (true)
            {
                ...
                var (_, agentCnt, obstacleCnt, seeQueen) =
                    GetEnvInfo(ref colliders, ref obstacles, ref agents, ref queen);

                // avoid obstacle
                Vector3 v0 = AvoidObstacle(obstacles, obstacleCnt);
        ...
        private (int colliderCnt, int agentCnt, int obstacleCnt, bool seeQueen) GetEnvInfo(ref Collider[] colliders,
            ref Collider[] obstacles, ref Collider[] agents, ref Collider queen)
        {
            int colliderCnt = 0, agentCnt = 0, obstacleCnt = 0;
            bool seeQueen = false;
            ...
            return (colliderCnt, agentCnt, obstacleCnt, seeQueen);
        }

        private Vector3 AvoidObstacle(Collider[] obstacles, int obstacleCnt)
        {
            ...
            if (obstacleCnt != 0)
            {
                ...
                var useFallOut =
                    Physics.Raycast(_transform.position, _rigidbody.velocity, _agentManager.VisibleRadius,
                        1 << 6)
                        ? _agentManager.AvoidObstacleFallOut
                        : _agentManager.DistObstacleFallOut;
                for (int i = 0; i < obstacleCnt; i++)
                {
                    Vector3 disVec = _transform.position - obstacles[i].transform.position;
                    float m = Vector3.Dot(obstacles[i].transform.forward, disVec);
                    v0 += obstacles[i].transform.forward *
        ...
        private Vector3 QueenEffect(bool seeQueen, Collider queen)
        {
            ...
            if (seeQueen)
            {
                ...
                v = disVec.normalized *
                    (_agentManager.QueenEffectFallOut.Evaluate(disVec.magnitude / _agentManager.VisibleRadius) *
                     _agentManager.QueenPullForce);
            }
